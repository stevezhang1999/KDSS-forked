...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i <= execution_time; i++)
    {
        ...
        _CXX_MEASURE_TIME(executed = computation_worker.Compute("mnist", d_input.get(), d_output_ptr), fout[0]);
        if (executed != 0)
        {
        ...
        _CXX_MEASURE_TIME(execution = computation_worker.ComputeWithStream("mnist", d_input.get(), d_output_ptr), fout[1]);
        if (executed != 0)
        {
            gLogError << __CXX_PREFIX << "Compute failed, exit..."
                      << endl;
